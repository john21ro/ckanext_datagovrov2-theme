...
from ckanext.datagovrov2_theme.logic.auth.delete import (
    datagovrov2_theme_package_delete, datagovrov2_theme_resource_delete)
...
class Datagovrov2_themePlugin(plugins.SingletonPlugin, DefaultTranslation):
    ...
    def after_map(self, mapping):
        GA_CONTROLLER = """
            ckanext.datagovrov2_theme.controllers.gacontroller:GAController"""
        with SubMapper(mapping, controller=GA_CONTROLLER) as m:
            m.connect('ga_index',
                      '/datagovrov2_theme/ga',
                      action='index')
        return mapping
    # IConfigurer
    def update_config(self, config):
        ...
        toolkit.add_resource('fanstatic', 'datagovrov2_theme')
    ...
    def update_config_schema(self, schema):
        ...
        schema.update({
            'ckanext.datagovrov2_theme.disallowed_extensions': [ignore_missing, unicode],
            'ckanext.datagovrov2_theme.allowed_extensions': [ignore_missing, unicode],
        })
    ...
    def before_create(self, context, resource):
        disallowed_extensions = toolkit.aslist(config.get('ckanext.datagovrov2_theme.disallowed_extensions',[]))
        disallowed_mimetypes = [mimetypes.types_map["." + x] for x in disallowed_extensions]

        allowed_extensions = toolkit.aslist(config.get('ckanext.datagovrov2_theme.allowed_extensions',[]))
    ...
    def before_show(context, resource_dict):
        custom_url = config.get('datagovrov2_theme.custom_resource_download_url')
    ...
    def get_auth_functions(self):
        return {
            'package_delete': datagovrov2_theme_package_delete,
            'resource_delete': datagovrov2_theme_resource_delete,
        }
